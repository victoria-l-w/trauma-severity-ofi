make_stats <-function(df) {
  ...
  ## ici.ci <- ici_ci(df) ## takes 15+ mins
  ...
  stats.ret <- list(model = model, roc = roc, auc = auc, acc = acc, ici = ici, auc.ci = auc.ci, or = or, or.p = or.p, acc.nrs = acc.nrs, acc.ci = acc.ci, ici.ci = ici.ci)
  return(stats.ret)
}
