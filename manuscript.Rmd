...
Detailed totals of missing data for each required parameter in cases that were excluded for incomplete data are given in table \@ref(tab:incomplete-table). Some patients had missing data in several parameters and therefore the total number of missing parameters is greater than the total number of patients excluded due to incomplete data.

```{r incomplete-table, echo=FALSE, fig.align = "center"}
knitr::kable(table(na.data, type = "incomplete"), caption = "Parameters that were missing in cases excluded for incomplete data") %>% kable_styling(position = "center", full_width = FALSE)
```
...
```{r table-one} 
  table.one
## this thing breaks the table counting but ill just fix it manually in word
```
